...
<head>
    ...
    <script type="text/javascript"
            src="/crm/jquery/bootstrap-datetimepicker-master/js/bootstrap-datetimepicker.min.js"></script>
    <script type="text/javascript"
...
<form class="form-horizontal" id="updateForm" role="form" style="position: relative; top: -30px;">
    <%--隐藏域--%>
...
<script>
    ...
    $.post("/crm/workbench/transaction/queryTransaction", {
        ...
        //把id放进隐藏域中
        $("#tranId").val(data.id);
        $("#aid").val(data.activityId);
        $("#cid").val(data.contactsId);

    }, 'json');
    //点击更新按钮   更新用户信息
    function updateTran() {
        var form = $("#updateForm").serialize();
        $.post("/crm/workbench/transaction/updateTran", form, function (data) {
            if (data.ok) {
                //更新成功  返回index页面
