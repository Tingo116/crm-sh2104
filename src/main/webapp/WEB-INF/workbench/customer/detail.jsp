...
<script>
	//显示详情页
	$.get("/crm/workbench/customer/toDetail",{
		'id':'${customerId}'
	},function(data){
	//	拼详情页
		//显示客户详细信息
		...
		for (var i = 0;i <customerRemarks.length;i ++){
			...
			appendRemark(customerRemark);
		}
	},'json');

	//抽取出一个拼接备注的方法
	function appendRemark(customerRemark) {
		$("#remarkDiv").before("<div class=\"remarkDiv\" id="+customerRemark.id+"remarkDiv style=\"height: 60px;\">\n" +
				...
				"\t\t\t\t<font color=\"gray\">联系人</font> <font color=\"gray\">-</font> <b>"+customerRemark.name+"-"+customerRemark.customerId+"</b> <small style=\"color: gray;\">"+customerRemark.createTime+" 由"+customerRemark.createBy+"</small>\n" +
				"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
				"\t\t\t\t\t<a class=\"myHref\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
				"\t\t\t\t\t&nbsp;&nbsp;&nbsp;&nbsp;\n" +
	...
	//客户备注的添加  需要传递的参数 客户id  还有添加备注的内容
	function addRemark() {
		//发送保存用户的请求
		$.get("/crm/workbench/customer/addRemark",{
	...
	function deleteCustomerRemark(id) {
		// 确认删除
		layer.msg('确定删除该条客户备注吗', {
			time: 0 //不自动关闭
			,btn: ['确定', '取消']
			,yes: function(index){
				layer.close(index);
				$.get("/crm/workbench/customer/deleteCustomerRemark",{
					'id':id,
				},function (data) {
					if (data.ok){
						layer.alert(data.mess, {icon: 6});
