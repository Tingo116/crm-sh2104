	...
	<div style="position:  relative; left: 30px;">
		<h3>新增字典值</h3>
	  	<div style="position: relative; top: -40px; left: 70%;">
			<button type="button" onclick="addDicvalue($('#addForm'))" class="btn btn-primary">保存</button>
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				<select class="form-control" name="typeCode" id="create-dicTypeCode" style="width: 200%;">
					...
					<%--<c:forEach items="${dics[typeCode]}" var="dic">
						<option value="${dic.typeCode}">${dic.value}</option>
					</c:forEach>--%>
...
<script>
	<%--字典值的添加--%>
	function addDicvalue(form) {
		var form = form.serialize();
		$.get("/crm/settings/dicValue/addValue",form,function (data) {
			if(data.ok){
